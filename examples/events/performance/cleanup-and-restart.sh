...
COMPOSE_DIR="$SCRIPT_DIR/../generated"
...
docker volume rm $(docker volume ls -q | grep -E "events.*mongo|events.*kafka|events.*redpanda" || true) 2>/dev/null || true
...
until curl -s http://localhost:4055/event/api/health > /dev/null 2>&1; do
